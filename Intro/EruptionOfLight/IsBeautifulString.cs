...
namespace Intro.EruptionOfLight {
    ...
    public class IsBeautifulString {
        ...
        public static bool solution(string inputString) {
            var groupedAndOrderedInputString = inputString.Distinct().OrderBy(x => x);

            foreach (var ch in groupedAndOrderedInputString) {
                if (ch == 'a') {
                    continue;
                }

                var currentCharCount = inputString.Count(c => c == ch);
                var previousCharCount = inputString.Count(c => c == ch - 1);
