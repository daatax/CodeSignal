...
namespace Intro.EruptionOfLight {
    ...
    /// If two or more candidates receive the same (maximum) number of votes, assume 
    ...
    /// (3 + 3 = 6 > 5).
    /// 
    /// The third candidate can win even if none of the remaining candidates vote f
    /// or him. For example, if each of the remaining voters cast their votes for 
    /// each of his opponents, he will still be the winner 
    ...
    public class ElectionsWinners {
        public static void Main(string[] args) {
            int[] votes = [2, 3, 5, 2];
            int k = 3;
            Console.WriteLine(solution(votes, k));
        }

        public static int solution(int[] votes, int k) {
            int potentialWinners = 0;
            int currentWinner = votes.Max();
            int currentWinnersCount = votes.Count(x => x == currentWinner);

            if (k == 0 && currentWinner == 1) {
                return 1;
            }
